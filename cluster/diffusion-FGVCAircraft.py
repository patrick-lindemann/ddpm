...
if os.path.exists('model_FGVCAircraft_checkpoint') and os.path.isdir('model_FGVCAircraft_checkpoint'):
    model = UNet2DModel().from_pretrained("model_FGVCAircraft_checkpoint")

model.to(device)

diffuser = diffusion(timesteps=TIME_STEPS)

image_data = torchvision.datasets.FGVCAircraft(
